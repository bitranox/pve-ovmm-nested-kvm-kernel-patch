...
"\t * HvSignalEvent from a nested Hyper-V root) carries an L2 GPA in ingpa.\n"
"\t * Translate it to an L1 GPA, exactly like the L2 TLB-flush slow path in\n"
"\t * kvm_hv_flush_tlb(), so the in-kernel read and the userspace post path\n"
"\t * see the right page. The walk also filters pages removed from the L2\n"
"\t * root's GPA space: it returns INVALID_GPA, which we reject. Done\n"
"\t * synchronously in the faulting vCPU's exit context and never cached.\n"
"\t */\n"
